...
// router.post("/", upload.array("attachment", 2), (req, res) => {
...
router.post("/:id/reply", requireAuth, (req, res) => {
    ...
    res.redirect(`/contactus/${reportId}`); // Redirect back to the report detail page after submitting the reply
});

router.put("/:id/:replyId/reply", requireAuth, (req, res) => {
    const reportId = req.params.id;
...
router.delete("/:id/:replyId/reply", requireAuth, (req, res) => {
    const reportId = req.params.id;
    const replyId = req.params.replyId;
